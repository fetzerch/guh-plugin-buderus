...
DeviceManager::DeviceSetupStatus DevicePluginBuderus::setupDevice(Device *device)
{
    ...
    auto host = device->paramValue(buderusHostParamTypeId).toString();
    auto port = device->paramValue(buderusPortParamTypeId).toString();
...
void DevicePluginBuderus::sendAsyncRequest(const Request &request)
{
    ...
    auto port = request.device->paramValue(buderusPortParamTypeId).toString();
